...
func (service *RegisterServiceReq) valid() *vo.Response {
	...
	count := 0
	entity.DB.Model(&entity.User{}).Where("phone = ?", service.Phone).Count(&count)
	if count > 0 {
		return &vo.Response{
			Code: 40001,
			Msg:  "手机已被注册",
	...
	count = 0
	entity.DB.Model(&entity.User{}).Where("identity_card = ?", service.IdentityCard).Count(&count)
	if count > 0 {
		return &vo.Response{
			Code: 40001,
			Msg:  "身份证已注册",
